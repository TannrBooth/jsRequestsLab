...
let queryDisplay = document.querySelector(`#query-button`)

queryDisplay.addEventListener('click',queryFunc)
...
const createFood = () => {
    let foodInput = document.querySelector('#food-form')
        ...
        axios.post('http://localhost:3000/food', body)
            .then((res) => {
                console.log(res.data)
                foodText = document('#food-form')
                foodText.textContent = res.data;
...
let foodFunc = document.querySelector('#food-form')

foodFunc.addEventListener('submit',createFood)